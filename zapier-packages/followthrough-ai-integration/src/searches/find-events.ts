...
const perform = async (z: ZObject, bundle: Bundle): Promise<EventSearchResult[]> => {
  const inputData = bundle.inputData as unknown as FindEventsInput;
  const { 
    summary, 
    description, 
    attendee, 
  ...
  try {
    ...
    if (attendee) {
      filteredEvents = filteredEvents.filter(event => 
        event.attendees?.some(att => att.toLowerCase().includes(attendee.toLowerCase()))
      );
    }
