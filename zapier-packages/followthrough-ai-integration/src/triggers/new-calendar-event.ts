...
const performList = async (z: ZObject, bundle: Bundle): Promise<CalendarEventTrigger[]> => {
  ...
  try {
    ...
    const events = await googleClient.getRecentCalendarEvents({
      ...
      timeMin: inputData.timeMin as string || new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
      timeMax: inputData.timeMax as string,
    });
