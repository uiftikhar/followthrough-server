...
export class SupervisorGraphBuilder extends BaseGraphBuilder<SupervisorState> {
   ...
   */
  private async processingNode(state: SupervisorState): Promise<SupervisorState> {
    try {
      const team = state.routing?.team || 'unknown';
      this.logger.log(`Processing input for session ${state.sessionId} with team ${team}`);
   ...
   */
  private async finalizationNode(state: SupervisorState): Promise<SupervisorState> {
    try {
