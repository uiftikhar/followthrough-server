...
Please analyze the sentiment of this meeting transcript and provide a comprehensive sentiment analysis in the specified JSON format.`;
  ...
  private processSentimentResult(result: any): SentimentAnalysis {
    ...
    try {
      ...
      if (typeof result === 'string') {
        ...
        this.logger.log(`Attempting to parse JSON: ${jsonStr.substring(0, 200)}...`);
  ...
  private validateSentimentAnalysis(result: any): SentimentAnalysis {
    ...
    const segments = Array.isArray(result.segments) ? result.segments.map(segment => ({
      text: segment.text || '',
      sentiment: validOverallSentiments.slice(0, 3).includes(segment.sentiment) ? segment.sentiment : 'neutral', // only positive, negative, neutral for segments
    ...
    const keyEmotions = Array.isArray(result.keyEmotions) ? result.keyEmotions.filter(emotion => 
    ...
    const toneShifts = Array.isArray(result.toneShifts) ? result.toneShifts.filter(shift => 
      shift && typeof shift.from === 'string' && typeof shift.to === 'string'
    ).map(shift => ({
      from: shift.from,
      to: shift.to,
    ...
    this.logger.log(`Validated sentiment: overall=${overall}, score=${score}, segments=${segments.length}, emotions=${keyEmotions.length}, shifts=${toneShifts.length}`);
  ...
  protected async executeLlmRequest(prompt: string, state: any): Promise<any> {
    try {
      ...
      // Invoke the LLM
      const messages = [
        { role: 'system', content: 'You are an expert at analyzing sentiment in meeting transcripts. Always respond with valid JSON in the exact format requested.' },
