...
export class GraphExecutionService {
  ...
  async executeGraph<T>(graph: any, initialState: T): Promise<T> {
    ...
    while (currentNode !== '__end__') {
      ...
      try {
        ...
        this.logger.error(`Error executing node ${currentNode}: ${error.message}`, error.stack);
        throw error;
      }
    }
   ...
   */
  private findNextNode(graph: any, currentNode: string, state: any): { target: string } | null {
  ...
  attachProgressTracker(graph: any, sessionId: string): void {
    this.logger.debug(`Attaching progress tracker to graph for session ${sessionId}`);
    ...
    graph.addStateTransitionHandler(async (prevState: any, newState: any, nodeName: string) => {
      try {
        ...
        if (progress > 0 && progress > (this.progressMap.get(sessionId) || 0)) {
