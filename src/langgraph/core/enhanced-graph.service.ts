...
export class EnhancedGraphService {
      ...
      async execute(initialState: any) {
        ...
        while (currentNode !== '__end__') {
          ...
          try {
            ...
            throw new Error(`Error executing node ${currentNode}: ${error.message}`);
          }
        }
  ...
  async buildMasterSupervisorGraph(): Promise<any> {
    ...
    const meetingAnalysisTeamNode = async (state: any) => {
      ...
      const teamHandler = this.teamHandlerRegistry.getHandler('meeting_analysis');
      ...
      try {
        ...
        this.logger.error(`Error in meeting analysis: ${error.message}`, error.stack);
        return {
          ...state,
          error: {
    ...
    const emailTriageTeamNode = async (state: any) => {
      ...
      try {
        ...
        this.logger.error(`Error in email triage: ${error.message}`, error.stack);
        return {
          ...state,
          error: {
            message: error.message,
    ...
    graph.addNode(this.masterNodeNames.MEETING_ANALYSIS_TEAM, meetingAnalysisTeamNode);
