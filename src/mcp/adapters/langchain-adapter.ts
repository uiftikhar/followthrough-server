...
export class LangchainMcpAdapter {
   ...
   */
  async createMultiServerClient(serverConfigs: {
    ...
    try {
      ...
      this.logger.log(
        `Creating MultiServerMCPClient with ${Object.keys(serverConfigs).length} servers`,
      );
      return new MultiServerMCPClient(config);
    } catch (error) {
      this.logger.error(
        `Failed to create MultiServerMCPClient: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async loadAllTools(client: MultiServerMCPClient): Promise<any[]> {
    try {
      ...
      this.logger.error(
        `Failed to load tools from MultiServerMCPClient: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async loadToolsFromServers(
    ...
    try {
      ...
      this.logger.error(
        `Failed to load tools from servers ${serverNames.join(", ")}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async createMcpClient(
    serverUrl: string,
    clientName: string = "langchain-mcp-client",
  ): Promise<Client> {
    try {
  ...
  async loadServerTools(serverName: string, client: Client): Promise<any[]> {
    try {
      ...
      this.logger.error(
        `Failed to load tools from server ${serverName}: ${error.message}`,
      );
      return [];
    }
  }
