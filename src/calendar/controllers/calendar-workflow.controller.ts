...
} from "../services/calendar-webhook.service";
...
export class CalendarWorkflowController {
  ...
  async getUpcomingEvents(
    ...
    const userId = req.user?.userId || req.user?.id;
    const events =
      await this.calendarSyncService.getEventsHappeningSoon(userId);
    return { events };
  }
  ...
  async getEventsSoon(@Request() req): Promise<{ events: CalendarEvent[] }> {
    ...
    const soonEvents = events.filter(
      (event) => new Date(event.startTime) <= twoHoursFromNow,
    );
   ...
   */
  @Get("events/next")
  async getNextEvent(@Request() req): Promise<{
  ...
  async setupPushNotifications(@Request() req): Promise<{
    ...
    try {
      const channel =
        await this.googleCalendarService.setupPushNotifications(userId);
  ...
  async getNotificationStatus(@Request() req): Promise<{
    ...
    try {
      const status =
        await this.googleCalendarService.checkChannelStatus(userId);
  ...
  async handleOutlookWebhook(
    @Body() notification: OutlookWebhookNotification,
  ): Promise<{ status: string }> {
