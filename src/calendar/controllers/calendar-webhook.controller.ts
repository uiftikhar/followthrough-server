import { Controller, Post, Body, Headers, Logger, HttpStatus, Res, HttpException } from '@nestjs/common';
import { Response } from 'express';
import { GoogleCalendarService, GoogleWebhookNotification } from '../services/google-calendar.service';
...
export class CalendarWebhookController {
  ...
  async handleGoogleWebhook(
    ...
    try {
      ...
      const notification: GoogleWebhookNotification = {
        ...
        resourceState: (headers['x-goog-resource-state'] || 'exists') as 'sync' | 'exists' | 'not_exists',
      ...
      this.logger.log(`📋 Processing webhook - Channel: ${notification.channelId}, State: ${notification.resourceState}`);
      ...
      const result = await this.googleCalendarService.processWebhookNotification(notification);
      this.logger.log(`✅ Found ${result.eventsChanged.length} changed events for user ${result.userId}`);
      ...
      if (result.eventsChanged.length > 0) {
        await this.eventDetectionService.processEventChanges(result.userId, result.eventsChanged);
      }
  ...
  async verifyGoogleWebhook(
    ...
    try {
      ...
      this.logger.log(`Verifying channel: ${channelId} with token: ${token ? 'Present' : 'Missing'}`);
      ...
      this.logger.error(`❌ Error verifying Google webhook: ${error.message}`, error.stack);
