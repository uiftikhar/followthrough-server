...
Respond with valid JSON following the specified interface structures.`,
  ...
  async generateMeetingBrief(
    ...
    try {
      ...
      const objectives = await this.generateMeetingObjectives(
        upcomingMeeting,
        meetingContext,
        finalOptions,
      );
      ...
      const participantPreparations = finalOptions.includeParticipantPrep
        ? await this.generateParticipantPreparations(
            upcomingMeeting,
            meetingContext,
            enhancedAgenda,
          )
      ...
      const timeManagement = finalOptions.includeTimeManagement
        ? await this.createTimeManagementPlan(
            upcomingMeeting,
            enhancedAgenda,
            finalOptions,
          )
      ...
      const deliveryOptions = finalOptions.includeDeliveryFormats
        ? await this.createDeliveryFormats(
            upcomingMeeting,
            objectives,
            enhancedAgenda,
            participantPreparations,
      ...
      const meetingBrief: MeetingBrief = {
        ...
        generationMetadata: {
          ...
          confidence: this.calculateBriefConfidence(
            meetingContext,
            enhancedAgenda,
            participantPreparations,
          ),
      ...
      this.logger.log(
        `Successfully generated meeting brief ${briefId} with ${enhancedAgenda.length} agenda items and ${participantPreparations.length} participant preparations`,
      );

      return meetingBrief;
    } catch (error) {
      this.logger.error(`Error generating meeting brief: ${error.message}`);
      return this.createMinimalBrief(
        upcomingMeeting,
        meetingContext,
        error.message,
      );
  ...
  private async enhanceAgenda(
    ...
    try {
      ...
      const existingAgenda = this.parseExistingAgenda(
        meeting.description || "",
      );

      const prompt = this.buildAgendaEnhancementPrompt(
        meeting,
        context,
        objectives,
        existingAgenda,
  ...
  private async generateParticipantPreparations(
    ...
    for (const attendee of meeting.attendees) {
      try {
        const participantHistory = context.participantHistories.find(
          (p) => p.email === attendee.email,
        );

        const prompt = this.buildParticipantPrepPrompt(
          attendee,
          participantHistory,
          agenda,
          context,
        ...
        const response = await this.processMessage(prompt);
        const preparation = this.parseParticipantPreparation(
          response,
          attendee,
        );
        ...
        this.logger.warn(
          `Error generating preparation for ${attendee.email}: ${error.message}`,
        );
  ...
  private async createTimeManagementPlan(
    ...
    try {
      ...
      const prompt = this.buildTimeManagementPrompt(
        meeting,
        agenda,
        duration,
        options,
  ...
  private async createDeliveryFormats(
    ...
    try {
      const prompt = this.buildDeliveryFormatsPrompt(
        meeting,
        objectives,
        agenda,
        preparations,
...
- Recent meetings: ${history.recentMeetings
        .slice(0, 3)
        .map((m) => m.title)
        .join(", ")}
`
...
Return valid JSON with email, slack, and calendar objects.`;
  ...
  private parseParticipantPreparation(
    response: string,
    attendee: any,
  ): ParticipantPreparation {
    try {
      ...
      this.logger.warn(
        `Error parsing participant preparation: ${error.message}`,
      );
      return this.createMinimalParticipantPrep(attendee);
    }
  }

  private parseTimeManagement(
    response: string,
    duration: number,
  ): TimeManagement {
    try {
  ...
  private parseDeliveryFormats(response: string): any {
    try {
      ...
      return this.createBasicDeliveryFormats({
        title: "Meeting",
      } as CalendarEvent);
    }
  }
  ...
  private extractContextualInsights(context: MeetingContext): any {
    return {
      relevantHistory: context.previousMeetingContext
        .slice(0, 5)
        .map(
          (m) =>
  ...
  private createExecutiveSummary(
    ...
    return {
      ...
      preparation: agenda.some(
        (a) => a.preparationRequired && a.preparationRequired.length > 0,
      )
        ? "Preparation required - see participant guidelines"
        : "Minimal preparation required",
      duration,
      complexity:
        duration > 90 || agenda.length > 6
          ? "high"
          : duration > 45 || agenda.length > 4
            ? "medium"
            : "low",
    };
  }
  ...
  private extractCustomizations(
    ...
    if (options.complexity !== "standard")
      customizations.push(`complexity: ${options.complexity}`);
    if (options.focusAreas.length !== 3)
      customizations.push(`focus: ${options.focusAreas.join(",")}`);
    if (options.maxBriefLength !== 2000)
      customizations.push(`length: ${options.maxBriefLength}`);
    if (!options.prioritizeActionItems)
      customizations.push("no action item priority");
  ...
  async processState(state: any): Promise<any> {
    ...
    if (!context) {
      this.logger.warn(
        "No meeting context found in state for brief generation",
      );
      return {
        ...state,
        meetingBrief: null,
    ...
    const meetingBrief = await this.generateMeetingBrief(
      meeting,
      context,
      options,
    );
