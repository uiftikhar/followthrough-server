...
Respond with valid JSON following the specified interface structures.`,
  ...
  async orchestrateFollowUp(meetingAnalysis: MeetingAnalysisResult): Promise<FollowUpPlan> {
    this.logger.log(`Orchestrating follow-up for meeting: ${meetingAnalysis.meetingTitle}`);

    try {
      const planId = `followup-${meetingAnalysis.meetingId}-${Date.now()}`;

      // Step 1: Analyze follow-up requirements
      const followUpRequirements = await this.analyzeFollowUpRequirements(meetingAnalysis);

      // Step 2: Generate email follow-ups
      const emailFollowUps = await this.generateEmailFollowUps(meetingAnalysis, followUpRequirements);

      // Step 3: Generate meeting follow-ups
      const meetingFollowUps = await this.generateMeetingFollowUps(meetingAnalysis, followUpRequirements);
      ...
      this.logger.log(`Successfully orchestrated follow-up plan ${planId} with ${emailFollowUps.length} emails, ${meetingFollowUps.length} meetings, ${taskFollowUps.length} tasks`);
  ...
  async routeActionItems(actionItems: ActionItem[]): Promise<RoutingResult> {
    this.logger.log(`Routing ${actionItems.length} action items to appropriate workflows`);
    ...
    for (const actionItem of actionItems) {
      try {
        ...
        const routingSuccess = await this.routeToWorkflow(actionItem, targetWorkflow);
        ...
        this.logger.warn(`Error routing action item ${actionItem.id}: ${error.message}`);
        routingDetails.push({
          actionItemId: actionItem.id,
    ...
    const successfulRoutes = routingDetails.filter(r => r.status === 'success').length;
    const failedRoutes = routingDetails.filter(r => r.status === 'failed').length;
  ...
  async generateFollowUpEmails(actionItems: ActionItem[]): Promise<EmailDraft[]> {
    this.logger.log(`Generating follow-up emails for ${actionItems.length} action items`);
    ...
    for (const [assignee, items] of Object.entries(groupedByAssignee)) {
      try {
        ...
        this.logger.warn(`Error creating email for ${assignee}: ${error.message}`);
      }
    }
  ...
  async scheduleFollowUpMeetings(schedulingItems: SchedulingItem[]): Promise<SchedulingResult> {
    ...
    for (const item of schedulingItems) {
      try {
        ...
        this.logger.warn(`Error scheduling meeting for ${item.id}: ${error.message}`);
        schedulingDetails.push({
          schedulingItemId: item.id,
          status: 'failed',
          error: error.message
    ...
    const successfulSchedules = schedulingDetails.filter(s => s.status === 'scheduled').length;
    const failedSchedules = schedulingDetails.filter(s => s.status === 'failed').length;
  ...
  private async analyzeFollowUpRequirements(meetingAnalysis: MeetingAnalysisResult): Promise<any> {
...
Determine what follow-up actions are needed and respond with JSON containing followUpRequirements analysis.`;
  ...
  private async generateTaskFollowUps(meetingAnalysis: MeetingAnalysisResult): Promise<TaskFollowUp[]> {
  ...
  private calculateOrchestrationMetadata(
    ...
    const totalActions = emailFollowUps.length + meetingFollowUps.length + taskFollowUps.length;
  ...
  private async determineTargetWorkflow(actionItem: ActionItem): Promise<string> {
    if (actionItem.task.toLowerCase().includes('email') || actionItem.task.toLowerCase().includes('contact')) {
      return 'email_triage';
    } else if (actionItem.task.toLowerCase().includes('meeting') || actionItem.task.toLowerCase().includes('schedule')) {
  ...
  private async routeToWorkflow(actionItem: ActionItem, targetWorkflow: string): Promise<boolean> {
    this.logger.log(`Routing action item ${actionItem.id} to ${targetWorkflow}`);
    return true; // Simulate success
  }

  private groupActionItemsByAssignee(actionItems: ActionItem[]): Record<string, ActionItem[]> {
  ...
  private async createActionItemEmail(assignee: string, actionItems: ActionItem[]): Promise<EmailDraft> {
    return {
  ...
  private async scheduleFollowUpMeeting(item: SchedulingItem): Promise<boolean> {
  ...
  private determineIfApprovalRequired(
    ...
    const hasUrgentItems = emailFollowUps.some(e => e.priority === 'urgent') ||
  ...
  private getEarliestDeadline(actionItems: ActionItem[]): string {
    const dates = actionItems.map(item => new Date(item.dueDate)).filter(date => !isNaN(date.getTime()));
    if (dates.length === 0) return new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
  ...
  private parseFollowUpRequirements(response: string): any {
    try {
      ...
      return jsonMatch ? JSON.parse(jsonMatch[0]) : { needsEmails: true, needsFollowUpMeetings: false };
    } catch (error) {
      return { needsEmails: true, needsFollowUpMeetings: false, urgentItems: false };
    }
  }

  private createMinimalFollowUpPlan(meetingAnalysis: MeetingAnalysisResult, error: string): FollowUpPlan {
    return {
  ...
  async processState(state: any): Promise<any> {
    ...
    if (!meetingAnalysis) {
      this.logger.warn('No meeting analysis found in state for follow-up orchestration');
      return {
        ...state,
        followUpPlan: null,
