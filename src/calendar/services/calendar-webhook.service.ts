...
export class CalendarWebhookService {
  ...
  async registerWebhook(
    userId: string,
    config: Partial<CalendarWebhookConfig>,
  ): Promise<string> {
  ...
  async handleGoogleWebhook(
    ...
    try {
      ...
      const webhook = Array.from(this.webhooks.values()).find(
        (w) =>
          w.resourceId === notification.resourceId && w.provider === "google",
      );

      if (!webhook) {
        this.logger.warn(
          `No webhook configuration found for resource: ${notification.resourceId}`,
        );
        return;
      }
      ...
      this.logger.error(
        `Error handling Google webhook: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleOutlookWebhook(
    ...
    this.logger.log(
      `Handling Outlook webhook notification: ${notification.subscriptionId}`,
    );
    ...
    try {
      ...
      const webhook = Array.from(this.webhooks.values()).find(
        (w) =>
          w.resourceId === notification.subscriptionId &&
          w.provider === "outlook",
      );

      if (!webhook) {
        this.logger.warn(
          `No webhook configuration found for subscription: ${notification.subscriptionId}`,
        );
        return;
      }
      ...
      this.logger.error(
        `Error handling Outlook webhook: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async schedulePreMeetingBrief(
    ...
    const briefTime = new Date(
      new Date(calendarEvent.startTime).getTime() - 30 * 60 * 1000,
    ); // 30 minutes before
    const now = new Date();
    ...
    if (briefTime <= now) {
      ...
      this.logger.log(
        `Meeting ${calendarEvent.id} starts soon, generating brief immediately`,
      );
    ...
    this.logger.log(
      `Scheduling pre-meeting brief for ${calendarEvent.id} in ${Math.round(delayMs / 1000 / 60)} minutes`,
    );
   ...
   */
  private async triggerPreMeetingBrief(
    calendarEvent: CalendarEvent,
    userId: string,
  ): Promise<void> {
    this.logger.log(
      `🎯 Triggering pre-meeting brief for event ${calendarEvent.id}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `✅ Successfully triggered pre-meeting brief for ${calendarEvent.id}, session: ${result.sessionId}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Error triggering pre-meeting brief for ${calendarEvent.id}: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleMeetingStarted(
    ...
    this.logger.log(
      `🎯 Meeting started: ${calendarEvent.id} - ${calendarEvent.title}`,
    );
    ...
    if (this.scheduledBriefs.has(calendarEvent.id)) {
      ...
      this.scheduledBriefs.delete(calendarEvent.id);
      this.logger.log(
        `Cancelled pending brief for started meeting ${calendarEvent.id}`,
      );
    }
  }
  ...
  async handleMeetingEnded(
    ...
    this.logger.log(
      `🎯 Meeting ended: ${calendarEvent.id} - ${calendarEvent.title}`,
    );
    ...
    this.logger.log(
      `📝 Meeting ${calendarEvent.id} ended - waiting for transcript to become available`,
    );
  }
   ...
   */
  async handleTranscriptAvailable(
    calendarEvent: CalendarEvent,
    transcript: string,
    userId: string,
  ): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `✅ Successfully triggered meeting analysis for ${calendarEvent.id}, session: ${result.sessionId}`,
      );
      this.logger.log(
      ...
      this.logger.error(
        `❌ Error triggering meeting analysis for ${calendarEvent.id}: ${error.message}`,
        error.stack,
      );
    }
  }
   ...
   */
  private async simulateEventDetection(
    userId: string,
    provider: "google" | "outlook",
  ): Promise<void> {
    this.logger.log(
      `Simulating event detection for user ${userId} on ${provider}`,
    );

    // This would be replaced with actual calendar API calls to detect changes
    // For now, just log that an event was detected
    this.logger.log(
      `📅 Event detected for user ${userId} - would trigger appropriate workflow based on event type`,
    );
  }
