...
import { CalendarEvent, CalendarAuth, CalendarSyncStatus, CalendarWebhookConfig } from '../interfaces/calendar-event.interface';
...
export class GoogleCalendarService {
  ...
  private readonly activeChannels = new Map<string, GoogleCalendarChannelResponse>();
   ...
   */
  async setupPushNotifications(userId: string): Promise<GoogleCalendarChannelResponse> {
    ...
    try {
      ...
      this.logger.log(`Creating notification channel: ${channelId} -> ${webhookUrl}`);
      ...
      this.logger.log(`✅ Successfully set up push notifications for user ${userId}`);
      this.logger.log(`Channel ID: ${channelResponse.id}, Resource ID: ${channelResponse.resourceId}`);

      return channelResponse;

    } catch (error) {
      this.logger.error(`❌ Error setting up push notifications for user ${userId}: ${error.message}`, error.stack);
  ...
  async stopPushNotifications(userId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`✅ Successfully stopped push notifications for user ${userId}`);

    } catch (error) {
      this.logger.error(`❌ Error stopping push notifications for user ${userId}: ${error.message}`, error.stack);
   ...
   */
  async processWebhookNotification(notification: GoogleWebhookNotification): Promise<{
    eventsChanged: CalendarEvent[];
    userId: string;
  }> {
    this.logger.log(`Processing webhook notification: ${notification.channelId}`);
    this.logger.log(`Resource State: ${notification.resourceState}, Message Number: ${notification.messageNumber}`);
    ...
    try {
      ...
      switch (notification.resourceState) {
        case 'sync':
          this.logger.log(`📡 Sync notification received for user ${userId} - channel is now active`);
          return { eventsChanged: [], userId };

        case 'exists':
          this.logger.log(`📅 Calendar change detected for user ${userId} - fetching updated events`);
        ...
        default:
          this.logger.warn(`Unknown resource state: ${notification.resourceState}`);
          return { eventsChanged: [], userId };
      }

    } catch (error) {
      this.logger.error(`❌ Error processing webhook notification: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  private async fetchRecentChanges(userId: string): Promise<CalendarEvent[]> {
    ...
    try {
      ...
      this.logger.log(`📊 Found ${transformedEvents.length} recent events for user ${userId}`);

      return transformedEvents;

    } catch (error) {
      this.logger.error(`❌ Error fetching recent changes for user ${userId}: ${error.message}`, error.stack);
      return [];
    }
  }
   ...
   */
  async getEventsStartingSoon(userId: string, minutesAhead: number = 5): Promise<CalendarEvent[]> {
    this.logger.log(`Getting events starting soon for user ${userId} (within ${minutesAhead} minutes)`);
    ...
    try {
      ...
      this.logger.log(`🔔 Found ${transformedEvents.length} events starting soon for user ${userId}`);

      return transformedEvents;

    } catch (error) {
      this.logger.error(`❌ Error getting events starting soon for user ${userId}: ${error.message}`, error.stack);
      return [];
    }
  }
   ...
   */
  async getEventsEndedRecently(userId: string, minutesAgo: number = 5): Promise<CalendarEvent[]> {
    this.logger.log(`Getting events that ended recently for user ${userId} (within ${minutesAgo} minutes)`);
    ...
    try {
      ...
      this.logger.log(`📅 Found ${recentlyEndedEvents.length} recently ended events for user ${userId}`);

      return recentlyEndedEvents;

    } catch (error) {
      this.logger.error(`❌ Error getting recently ended events for user ${userId}: ${error.message}`, error.stack);
      return [];
    }
  }
   ...
   */
  async checkChannelStatus(userId: string): Promise<{ active: boolean; channel?: GoogleCalendarChannelResponse }> {
    const channel = this.activeChannels.get(userId);
    ...
    if (channel.expiration) {
      ...
      if (Date.now() > expirationTime) {
        this.logger.warn(`Channel ${channel.id} for user ${userId} has expired`);
        this.activeChannels.delete(userId);
        return { active: false };
      }
    }
   ...
   */
  async renewPushNotifications(userId: string): Promise<GoogleCalendarChannelResponse> {
