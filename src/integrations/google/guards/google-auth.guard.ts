...
export class GoogleAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (!isConnected) {
        this.logger.warn(`GoogleAuthGuard: User ${userId} not connected to Google`);
      ...
      const googleClient = await this.googleOAuthService.getAuthenticatedClient(userId);
      ...
      this.logger.error(`GoogleAuthGuard: Access denied for user ${userId}:`, error);
