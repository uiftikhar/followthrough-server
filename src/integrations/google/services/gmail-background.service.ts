...
export class GmailBackgroundService implements OnModuleInit {
  ...
  async forceRenewalCheck(): Promise<{
    ...
    try {
      ...
      const expiringWatches =
        await this.gmailWatchService.findWatchesNeedingRenewal();
      const details: any[] = [];
      let renewed = 0;
      let failed = 0;
      ...
      this.logger.log(
        `Force renewal completed: ${renewed} renewed, ${failed} failed`,
      );
  ...
  async forcePullProcessing(): Promise<{
    ...
    try {
      ...
      this.logger.log(
        `Force pull processing completed: ${notifications.length} processed`,
      );
  ...
  async getSystemHealth(): Promise<{
    ...
    try {
      ...
      try {
        ...
        const isHealthy =
          subscriptionHealth.pushSubscription?.exists &&
          subscriptionHealth.pullSubscription?.exists;
        checks.push({
          name: "Subscriptions",
      ...
      try {
        ...
        checks.push({
          ...
          details: {
            ...watchStats,
            contextNote:
              "Only processing watches for users with active sessions",
          },
          timestamp: new Date().toISOString(),
        });
        ...
        this.logger.log(
          `📊 Background service health check: ${watchStats.totalActive} active watches, ${errorCount} with errors`,
        );
      } catch (error) {
        checks.push({
          name: "Gmail Watches",
      ...
      this.logger.log(
        `🏥 System health check completed: ${overallStatus} (${healthyChecks}/${totalChecks} checks passed)`,
      );
  ...
  private async performInitialHealthCheck(): Promise<void> {
    try {
      ...
      if (health.status === "unhealthy") {
        this.logger.warn(
          "System is unhealthy on startup - manual intervention may be required",
        );
      }
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        trends: {
          ...
          message:
            "Trend analysis will be available after collecting historical data",
        },
      };
    } catch (error) {
  ...
  private generateHealthRecommendations(health: any, stats: any): string[] {
    ...
    if (stats.withErrors > 0) {
      recommendations.push(
        `${stats.withErrors} watches have errors - review and fix`,
      );
    }

    if (stats.expiringSoon > 0) {
      recommendations.push(
        `${stats.expiringSoon} watches are expiring soon - check renewal process`,
      );
    }

    if (stats.totalActive === 0) {
      recommendations.push(
        "No active watches found - users may need to re-enable notifications",
      );
    }
