...
export class GmailNotificationService {
  ...
  async processGmailNotification(notificationData: any): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`📬 Gmail notification decoded for: ${userEmail}, historyId: ${historyId}`);
      ...
      this.logger.log(`✅ Security validation passed for ${userEmail} - processing notification`);
      
      // Rest of the existing processing logic...
      await this.processUserNotification(userEmail, historyId);
      
      const processingTime = Date.now() - startTime;
      this.logger.log(`⚡ Gmail notification processed in ${processingTime}ms for: ${userEmail}`);
      
    } catch (error) {
      this.logger.error(`❌ Failed to process Gmail notification: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  private async validateActiveUserSession(userEmail: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`❌ Failed to validate user session for ${userEmail}: ${error.message}`);
  ...
  private async cleanupExpiredWatch(userEmail: string): Promise<void> {
    try {
      ...
      // Find watch by email
      const watch = await this.gmailWatchRepository.findByGoogleEmail(userEmail);
      if (!watch) {
      ...
      this.logger.error(`Failed to cleanup watch for ${userEmail}: ${error.message}`);
    }
  }
   ...
   */
  private async processUserNotification(userEmail: string, historyId: string): Promise<void> {
    // Implement the actual notification processing logic here
    this.logger.log(`Processing notification for ${userEmail} with historyId ${historyId}`);
    
    // Update watch statistics
    try {
      const watch = await this.gmailWatchRepository.findByGoogleEmail(userEmail);
      if (watch) {
        await this.gmailWatchRepository.incrementNotificationCount(watch.watchId);
      }
    } catch (error) {
  ...
  async comprehensiveCleanup(): Promise<{
    ...
    try {
      this.logger.log('🧹 Starting comprehensive cleanup of sessions and watches');
      ...
      const watchCleanupResult = await this.gmailWatchService.cleanupInactiveWatches(activeUserEmails);
  ...
  async validateNotificationSafety(userEmail: string): Promise<{
    ...
    try {
      ...
      if (!activeConnections) {
        ...
        this.logger.warn(`🚫 SECURITY: Notification for ${userEmail} rejected - no active sessions`);
      ...
      if (watch && watch.createdAt) {
        ...
        if (watchAge > maxAge) {
          this.logger.warn(`⏰ Notification for ${userEmail} rejected - watch too old (${Math.round(watchAge / (24 * 60 * 60 * 1000))} days)`);
      ...
      this.logger.error(`Failed to validate notification safety for ${userEmail}: ${error.message}`);
      return {
        shouldProcess: false,
