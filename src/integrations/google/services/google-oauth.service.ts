...
import {
  ...
  CreateGoogleTokensParams,
} from "../../../database/repositories/user-google-tokens.repository";
...
export class GoogleOAuthService {
  ...
  generateAuthUrl(userId: string): string {
    ...
    try {
      ...
      this.logger.error(
        `Failed to generate OAuth URL for user ${userId}:`,
        error,
      );
  ...
  async handleCallback(
    code: string,
    state: string,
  ): Promise<{ userId: string; userInfo: GoogleUserInfo }> {
  ...
  async getGoogleUserInfo(userId: string): Promise<GoogleUserInfo | null> {
    const userInfo =
      await this.userGoogleTokensRepository.getGoogleUserInfo(userId);
  ...
  async getAuthenticatedClient(userId: string): Promise<Auth.OAuth2Client> {
    const tokenData =
      await this.userGoogleTokensRepository.getTokensWithDecryption(userId);
  ...
  async refreshTokens(userId: string): Promise<void> {
    ...
    try {
      ...
      throw new UnauthorizedException(
        "Token refresh failed - please reconnect to Google",
      );
    }
  }
    ...
    try {
      ...
      if (tokenData) {
        ...
        try {
          ...
          this.logger.warn(
            `Google token revocation failed for user ${userId}:`,
            error,
          );
        }
      }
      ...
      this.logger.error(
        `Failed to revoke Google access for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async getTokenStatus(userId: string): Promise<{
    ...
    const tokenMetadata =
      await this.userGoogleTokensRepository.getTokenMetadata(userId);
  ...
  async refreshExpiringTokens(): Promise<void> {
    ...
    try {
      const expiringTokens =
        await this.userGoogleTokensRepository.getUsersWithExpiringTokens();

      this.logger.log(
        `Found ${expiringTokens.length} users with expiring tokens`,
      );
      ...
      for (const tokenDoc of expiringTokens) {
        try {
          ...
          this.logger.error(
            `Background refresh failed for user ${tokenDoc.userId}:`,
            error,
          );
        }
      }
