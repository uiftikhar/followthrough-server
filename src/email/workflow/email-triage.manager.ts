  ...
  async processEmail(emailData: any, context: any): Promise<EmailTriageResult> {
    ...
    try {
      ...
      const [classification, summary] = await Promise.all([
        ...
        this.summarizationAgent.summarizeEmail(emailData.body, emailData.metadata),
      ]);
      ...
      this.logger.log('Generating reply draft based on classification and summary');
      ...
      this.logger.log(`Email triage completed successfully for session: ${sessionId}`);
      return result;
    } catch (error) {
      this.logger.error(`Email triage failed for session ${sessionId}: ${error.message}`, error.stack);
