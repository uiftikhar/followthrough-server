...
export class Email {
  ...
  constructor(data: Partial<Email>) {
    ...
    this.from =
      data.from instanceof EmailAddress
        ? data.from
        : new EmailAddress(data.from || { address: "" });

    this.to = Array.isArray(data.to)
      ? data.to.map((to) =>
          to instanceof EmailAddress ? to : new EmailAddress(to),
        )
      : [];
    ...
    if (data.cc) {
      this.cc = Array.isArray(data.cc)
        ? data.cc.map((cc) =>
            cc instanceof EmailAddress ? cc : new EmailAddress(cc),
          )
        : [];
    }
    ...
    if (data.bcc) {
      this.bcc = Array.isArray(data.bcc)
        ? data.bcc.map((bcc) =>
            bcc instanceof EmailAddress ? bcc : new EmailAddress(bcc),
          )
        : [];
    }
    ...
    if (data.replyTo) {
      this.replyTo = Array.isArray(data.replyTo)
        ? data.replyTo.map((rt) =>
            rt instanceof EmailAddress ? rt : new EmailAddress(rt),
          )
        : [];
    }
    ...
    if (data.attachments) {
      this.attachments = Array.isArray(data.attachments)
        ? data.attachments.map((att) =>
            att instanceof EmailAttachment ? att : new EmailAttachment(att),
          )
        : [];
    }
