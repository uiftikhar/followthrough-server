...
export class EmailActionController {
  ...
  async sendReply(
    ...
    this.logger.log(`Sending reply for email ${emailId} by user ${req.user?.id}`);
    ...
    try {
      ...
      this.logger.error(`Failed to send reply for email ${emailId}: ${error.message}`, error.stack);
  ...
  async delegateEmail(
    ...
    this.logger.log(`Delegating email ${emailId} from ${req.user?.email} to ${delegateData.delegateToEmail}`);
    ...
    try {
      ...
      this.logger.log(`Email ${emailId} delegated successfully: ${delegationResult.id}`);
      ...
      this.logger.error(`Failed to delegate email ${emailId}: ${error.message}`, error.stack);
  ...
  async snoozeEmail(
    ...
    this.logger.log(`Snoozing email ${emailId} until ${snoozeData.snoozeUntil} by user ${req.user?.id}`);
    ...
    try {
      ...
      const snoozeResult = await this.emailSnoozeAgent.snoozeEmail(snoozeRequest);

      this.logger.log(`Email ${emailId} snoozed successfully: ${snoozeResult.id}`);
      ...
      this.logger.error(`Failed to snooze email ${emailId}: ${error.message}`, error.stack);
  ...
  async getActiveSnoozes(@Request() req: any) {
    ...
    try {
      const activeSnoozes = await this.emailSnoozeAgent.getActiveSnoozes(req.user.id);
      ...
      this.logger.error(`Failed to get active snoozes: ${error.message}`, error.stack);
  ...
  async getSnoozeStats(@Request() req: any) {
    ...
    try {
      ...
      this.logger.error(`Failed to get snooze stats: ${error.message}`, error.stack);
