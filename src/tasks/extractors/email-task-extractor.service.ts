...
export class EmailTaskExtractorService {
  ...
  constructor(
    ...
    this.strategies = [this.ruleBasedStrategy, this.llmStrategy];
  }
  ...
  async extractTasks(email: Email): Promise<Task[]> {
    try {
      ...
      for (const strategy of this.strategies) {
        ...
        if (tasks.length > 0) {
          this.logger.debug(
            `Extracted ${tasks.length} tasks using ${strategy.constructor.name}`,
          );
          break;
        }
      }
  ...
  async validateAndEnrichTask(task: Partial<Task>): Promise<Task> {
    try {
      ...
      if (!task.dueDate && task.description) {
        const extractedDate = await this.extractDueDateFromText(
          task.description,
        );
      ...
      const response = await model.invoke([
        {
          role: "system",
          content:
            'You are a date extraction assistant. Extract any due dates or deadlines mentioned in the text. Return ONLY an ISO date string (YYYY-MM-DD) or "null" if no date is found.',
        },
        {
          role: "user",
          content: text,
